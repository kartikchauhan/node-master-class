...
handler._check.get = function(data, callback) {
    ...
    if(id)    
    {
        _data.read('checks', id, function(err, checkData) {
            if(err)
            {
                callback(500, {'error': 'Error fetching check data'});
            }
            else
            {
                const token = typeof(data.headers.token) == 'string' && data.headers.token.trim().length == 20 ? data.headers.token.trim() : false;                
                handler._token.verifyToken(token, checkData.phone, function(checkToken) {
                    if(!checkToken)
                    {
                        callback(403, {'error': 'You are not authorized for the information you are trying to access'});
                    }
                    else
                    {
    ...
    {
        callback(400, {'error': 'Check id missing'});
    }
}; 
...
handler._check.put = function(data, callback) {
    ...
    if(id)    
    {
        ...
        const protocol = typeof(data.payload.protocol) == 'string' && ['https', 'http'].indexOf(data.payload.protocol) > -1 ? data.payload.protocol : false;
        const method = typeof(data.payload.method) == 'string' && ['post', 'get', 'put', 'delete'].indexOf(data.payload.method) > -1 ? data.payload.method : false;
        const timeoutSeconds = typeof(data.payload.timeoutSeconds) == 'number' && data.payload.timeoutSeconds > 0 && data.payload.timeoutSeconds <= 60 ? data.payload.timeoutSeconds : false;
        const url = typeof(data.payload.url) == 'string' && data.payload.url.trim().length > 0 ? data.payload.url.trim() : false;
        const successCodes = typeof(data.payload.successCodes) == 'object' && data.payload.successCodes instanceof Array ? data.payload.successCodes : false;
        ...
        _data.read('checks', id, function(err, checkData) {
            if(err)
            {
                callback(500, {'error': 'Error fetching current check information'});
            }
            else
            {       
                const token = typeof(data.headers.token) == 'string' && data.headers.token.trim().length == 20 ? data.headers.token.trim() : false;
